...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplay (songplay_id serial primary key, start_time varchar not null,
            user_id varchar not null, level text, songid text, artist_id text, session_id int, location text, user_agent text)""")

user_table_create = ("""CREATE TABLE IF NOT EXISTS users (userId varchar primary key, firstName text,lastName text, gender char, level text)""")

song_table_create = ("""CREATE TABLE IF NOT EXISTS song (songid varchar primary key, title varchar, artist_id text, year int, duration float)""")
